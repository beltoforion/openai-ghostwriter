...
class OpenAIGhostWriter:
    def __init__(self, output_folder : pathlib.Path, template_file : pathlib.Path):
        self.__api_key = os.environ["OPENAI_API_KEY"]
        self.__model = "text-davinci-003" # biggest, best and most expensive
#        self.__model = "text-curie-001" # max 2049 tokens
#        self.__model = "text-babbage-001" # max 2049 tokens
    ...
    def write_introduction(self, topic, chapter, subsection, nwords=100):
        if chapter is None:
            prompt = f'{topic}; Introductory text with at least {nwords} words; Format as html multiple <p> sections but no heading'
        elif subsection is None:
            prompt = f'Topic: {chapter} / {topic}; Introductory; {nwords} Words; Format as html multiple <p> sections but no heading'
        else:
    ...
    def create_toc(self, topic, nchapter):
        if self.__verbose:
            print(f'Creating table of contents "{topic}"')
    ...
    def write(self, topic, nchapter):
        ...
        # merge article into template
        path = {str(self.__template_file)}
...
#        tree = lxml.etree.ElementTree(lxml.etree.fromstring(toc))
        root = tree.getroot()
...
#    "Wie Aliens den 100 Jährigen Krieg beendeten"
#    "Verkannte Genies der Geschichte."
#    "Die Geschichte der mechanisierten Kriegführung."
#    "Der Untergang des Römischen Reiches"
#    "The entire history of mars"

    controller = OpenAIGhostWriter(output_folder, template_file)
